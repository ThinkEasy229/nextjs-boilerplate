...
function validateBody(
  input: unknown
): { ok: true; value: WrapConceptRequest } | { ok: false; error: string } {
  const candidate =
    typeof input === "object" && input !== null
      ? (input as Partial<Record<keyof WrapConceptRequest, unknown>>)
      : {};

  const vehicleType = normalizeString(candidate.vehicleType);
  const designDirection = normalizeString(candidate.designDirection);
  const companyName = normalizeString(candidate.companyName);
  const contactEmail = normalizeString(candidate.contactEmail);
